...
function getDateInfo() {
  ...
  var format = function(d) { return d.getDate() + ' ' + months[d.getMonth()] + ' ' + d.getFullYear() + ' ' + daysT[d.getDay()]; };
...
function nosyFetch(apiKey, citySlug, districtSlug, date) {
  var url = NOSY_BASE + 'pharmacies-on-duty?city=' + encodeURIComponent(citySlug);
...
function parsePharmacies(json) {
  ...
  return rows.map(function(p) {
    return {
      name:    p.pharmacyName || p.EczaneAdi || p.name    || '',
      dist:    p.district     || p.Ilce      || p.dist    || '',
      address: p.address      || p.Adres     || '',
      phone:   p.phone        || p.Telefon   || '',
      lat:     p.latitude     || p.Enlem     || p.lat     || '',
      lng:     p.longitude    || p.Boylam    || p.lng     || ''
    };
  });
}
...
app.get('/api/eczaneler', async function(req, res) {
  ...
  var districtSlug = (req.query.ilce  || '').trim();
  ...
  var cacheKey = citySlug + '|' + districtSlug + '|' + date;
  ...
  try {
    var r = await nosyFetch(apiKey, citySlug, districtSlug, date);
...
app.get('/api/test-nosyapi', async function(req, res) {
  ...
  try {
    var r = await nosyFetch(apiKey, 'istanbul', '', tarih);
    var text = await r.text();
    var parsed = null;
    try { parsed = JSON.parse(text); } catch(e) {}
    var rows = parsed && parsed.data;
    var firstRow = Array.isArray(rows) && rows.length > 0 ? rows[0] : null;
    res.json({ status: r.status, rowCount: parsed && parsed.rowCount, firstRowKeys: firstRow ? Object.keys(firstRow) : null, firstRow: firstRow });
